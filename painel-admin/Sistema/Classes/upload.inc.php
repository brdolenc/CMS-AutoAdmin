					...
					if($this->TamanhaArquivo()==true) {
							...
							if (move_uploaded_file($this->Arquivo['tmp_name'], $this->Pasta . $this->nome_final)) {
								chmod($this->Pasta . $this->nome_final, 0777);
									...
									$sql_up = mysql_query("INSERT INTO ".$this->Banco." SET id_princ= '".$this->IdPai."', arquivo = '".$this->nome_final."', data = NOW()")or(die('<div class="aviso erro"><p><strong>Sucesso!</strong>'.mysql_error().'</p></div>'));
