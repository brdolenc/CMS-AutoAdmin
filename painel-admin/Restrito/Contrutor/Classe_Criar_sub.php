    ...
    class Trata_variaveis
	{
	   ...
	   function Resgata($N_Referencia,$V_Referencia,$N_campo,$V_campo,$N_Tipo,$V_Tipo,$N_Tamanho,$V_Tamanho,$QTD,$Titulo_banco,$sub_ferramenta,$Galeria,$Upload,$Post_destaque,$Sitema_comente,$categorias)
	   {
		  ...
		  if($V_campo == true and $V_Tipo == true and $V_Tamanho) {
			  ...
			  $SQL_Cria .= "`id_sub` int(9) NOT NULL,";
			  ...
			  for($x=1;$x <= $QTD;$x++) {
				...
				mysql_query("INSERT INTO tbl_paginas SET nome_ferramenta = '$Titulo_banco', status = 'OFF', sub = '0', sub_de = '$sub_ferramenta'")or(die('<div class="aviso erro"><p>'.mysql_error().'</p></div>'));
			 			  
			  
			  	function Alterar_Sub($Ferramenta,$SubPrincipal){
			  
			  
			  	$Sql_PagesSub = mysql_query("SELECT * FROM tbl_paginas WHERE sub = '0' AND sub_de = '$SubPrincipal'")or(die('<div class="aviso erro"><p>'.mysql_error().'</p></div>'));
				$ContSubs = mysql_num_rows($Sql_PagesSub);
				
				$NovaSub = '$SubFerramentas['.$ContSubs.']="'.$Ferramenta.'";';
		
				$arquivo = fopen('Restrito/paginas/'.$SubPrincipal.'/'.$SubPrincipal.'.php','r+');
					if ($arquivo) {
						while(true) {
							$linha = fgets($arquivo);
							...
							if(preg_match('/SubFerramentas/', $linha)) {
		
								
								$string .= str_replace("$SubFerramentas = array();", "$SubFerramentas = array(); 
	  ".$NovaSub."", $linha);
			...
			$SubCriar_Status = Alterar_Sub($Titulo_banco,$sub_ferramenta);

			
			if($SubCriar_Status == true) { $RetornaSub = true; } else { $RetornaSub = false; }
				...
				include 'Restrito/Criar_paginas/Cria_categorias_sub.php';
				...
				include 'Restrito/Criar_paginas/Cria_galeria_sub.php';
				...
				include 'Restrito/Criar_paginas/Cria_upload_sub.php';
			   ...
			   include 'Restrito/Criar_paginas/Cria_comentarios_sub.php';
			  
			   }
			   

			   include 'Restrito/Criar_paginas/Cria_principal_sub.php';
			   include 'Restrito/Criar_paginas/Cria_cadastro_sub.php';
			   include 'Restrito/Criar_paginas/Cria_cadastro_restaurar_sub.php';
			   include 'Restrito/Criar_paginas/Cria_editar_sub.php';
			   include 'Restrito/Criar_paginas/Cria_mudar_sub.php';
			   include 'Restrito/Criar_paginas/Cria_editar_restaurar_sub.php';
